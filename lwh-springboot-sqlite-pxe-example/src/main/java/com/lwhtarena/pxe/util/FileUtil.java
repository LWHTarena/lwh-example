...
//    }
     ...
     * @param net
     */
    public void modifyLocalEth0(Net net) {
        /*******************************************
         * 修改ifcfg-eth0   service network restart
         *******************************************/
        ...
        try {
            ...
            infoLog.error("重启 service network restart 失败");
            e.printStackTrace();
        }
    ...
    public Net gatherInfo() {
        ...
        String line =null;
        try {
            br = new BufferedReader(new FileReader(Default.LOCAL_NET_ETH0));//
            while ((line =br.readLine())!=null){
                if(line.toUpperCase().startsWith("IPADDR")){
                    String str =getIp(line);
                    net.setIp(str);// ip
                }
                if(line.toUpperCase().startsWith("NETMASK")){
                    String str =getIp(line);
                    net.setNetmask(str);// ip
                }
                if(line.toUpperCase().startsWith("GATEWAY")){
                    String str =getIp(line);
                    net.setGateway(str);// ip
                }

            }
        } catch (FileNotFoundException e) {
            infoLog.info(">>>>>/etc/sysconfig/network-scripts/ifcfg-eth0 not exist ");
            e.printStackTrace();
        } catch (IOException e) {
            infoLog.info(">>>>> 文件流出错... ");
            e.printStackTrace();
