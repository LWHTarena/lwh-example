...
public class SearchPage {
    public static void main(String[] args) throws IOException,ParseException  {
        ...
        Query query = new QueryParser("sellPoint",new SmartChineseAnalyzer()).parse(keyWords);//模糊搜索

        //命中前10条文档
        TopDocs topDocs = indexSearcher.search(query,end);//根据end查询

        Integer totalPage = Math.toIntExact(((topDocs.totalHits / pageSize) == 0) ? (topDocs.totalHits / pageSize) : ((topDocs.totalHits / pageSize) + 1));
